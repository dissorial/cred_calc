...
class Neuronic:
    def __init__(self, cost, container):
        ...
        self.fst_cap = userInput_number(
            'Command cap/day', 1, 15, 9, 1, 'neuro_fst_cap', retInt=True)

        self.cost = cost
        self.container = container

        with container.expander('Neuronic activity', expanded=False):
            self.activity = userInput_number('Cycles per day', 0, math.floor(
                self.fst_cap), math.floor(self.fst_cap), 1, 'neuro_activity', retInt=True)

            self.activity_days = userInput_number(
                'Number of days', 1, 50, 2, 1, 'neuro_activity_days', retInt=True)
# ! VP--------------------------------------------------------
        with st.expander('SETTINGS: VP GAINS', expanded=False):
            self.fst_rate = (userInput_number(
                '!fight/!steal/!taunt success rate', 25.0, 100.0, 55.0, 2.5, 'neuro_success_rate')/100)

            self.fight_vp = (userInput_number(
                '!fight VP', 1, 300, 100, 5, 'neuro_fightVP'))

            self.steal_vp = userInput_number(
                '!steal VP', 1, 300, 100, 5, 'neuro_stealVP')

            self.tauntCall_vp = userInput_number(
                '!taunt #arena VP', 0, 300, 0, 5, 'neuro_tcVP')

            self.tauntRespond_vp = userInput_number(
                '#taunt response VP', 1, 300, 100, 5, 'neuro_trVP')

            self.vp_mp = userInput_number(
                'VP multiplier (%)', 0, 300, 40, 5, 'neuro_vp_mp', retInt=True)
...
# & CC--------------------------------------------------------
        with st.expander('SETTINGS: CC GAINS', expanded=False):
            self.fight_cc = userInput_number(
                '!fight CC', 1, 200, 60, 5, 'neuro_fight_cc')

            self.steal_cc = userInput_number(
                '!steal CC', 1, 200, 60, 5, 'neuro_steal_cc')

            self.tauntCall_cc = userInput_number(
                '!taunt #arena CC', 1, 100, 30, 5, 'neuro_tc_cc')

            self.tauntRespond_cc = userInput_number(
                '#taunt response CC', 1, 100, 30, 5, 'neuro_tr_cc')

            self.cc_mp = userInput_number(
                'Cyber Credit +%', 0, 300, 200, 5, 'neuro_poly_cc', retInt=True)
